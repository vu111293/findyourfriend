...
public class MapFragment extends BaseContainerFragment implements
	...
	private void afterViewCreate(View view) {
		...
		// load stranger and invited
		loadStrangerAndInvited();
							...
							Runnable mUpdateRunnable = new Runnable() {
								public void run() {
									...
									(new AsyncTask<Void, Void, Friend>() {
										...
										@Override
										protected Friend doInBackground(
												Void... params) {

											return PostData.friendGetFriend(
													getActivity(), k);
										}
	...
	private void setupFriendList() {
		...
		friendList.addAll(FriendManager.getInstance().getFriendsAndMine());

		swipeAdapter = new FriendSwipeAdapter(context,
				R.layout.item_friend_accepted, friendList);
	...
	private void changeVisibilityView() {
		...
		for (int i = 0; i < numOfButton; ++i) {
			...
			// Log.i(TAG, imgButtons[i].toString() + " # " +
			// imgButtons[i].getX()
			// + " @ " + imgButtons[i].getY());
		}
		...
		if (isToastPosition) {
			...
			(new AsyncTask<Void, Void, String>() {
				...
				protected String doInBackground(Void... arg0) {
					return Utility.getAddress(
							context,
							FriendManager.getInstance().hmMemberFriends.get(
									fIDCurrent).getLastLocation());
				}
	...
	private void loadStrangerAndInvited() {

		// refresh on map
		for (int fid : markerSIn.keySet()) {
			markerSIn.get(fid).remove();
		}
		markerSIn.clear();
		
		
		HashMap<Integer, Marker> tmpSIN = new HashMap<Integer, Marker>();
		
		// load on map
		for (int id : FriendManager.getInstance().hmStrangers.keySet()) {
			if (tmpSIN.keySet().contains(id)) {
				tmpSIN.get(id)
						.setPosition(
								new LatLng(
										FriendManager.getInstance().hmStrangers
	...
	private void updateFriendsSimple() {
		(new AsyncTask<Void, Void, Void>() {
			...
			protected Void doInBackground(Void... params) {
				FriendManager.getInstance().preLoadUpdate();
				return null;
			}
			...
			protected void onPostExecute(Void result) {
				...
				FriendManager.getInstance().endLoadUpdate();
		...
		public void onReceive(Context ctx, Intent intent) {
			...
			if (action.equals(com.sgu.findyourfriend.mgr.Config.MAIN_ACTION)) {
				...
				if (intent.getStringExtra(
					...
					friendList.clear();
					friendList.addAll(FriendManager.getInstance()
							.getFriendsAndMine());
					...
					if (intent.getStringExtra(
						...
						int fid = intent.getIntExtra(Config.FRIEND_ID, 0);
						if (fid > 0) {
							showDialogAddSuppoter(fid);
						} else {
					...
					Log.i("friend------UPDATE AFTER", "OK");
					friendList.clear();
					friendList.addAll(FriendManager.getInstance()
							.getFriendsAndMine());
					Log.i("fri", friendList.size() + "");
	...
	private void showDialogAddSuppoter(final int fid) {
		final Dialog dialog = new Dialog(getActivity());
		String name = FriendManager.getInstance().getNameFriend(fid);
		Utility.showDialog(
				Utility.CONFIRM,
				dialog,
				"Thêm bạn", name  + " đã chấp nhận yêu cầu kết bạn." + 
				" Bạn có muốn thêm " + name + " vào danh sách trợ giúp khi cần thiết?",
				"Có", new OnClickListener() {
					...
					public void onClick(View v) {
						...
						// add supporter
						Set<String> emerIDs = SettingManager.getInstance()
								.getDefaultWarning();
						emerIDs.add(fid + "");
						SettingManager.getInstance().setDefaultWarning(
								new HashSet<String>(emerIDs));
