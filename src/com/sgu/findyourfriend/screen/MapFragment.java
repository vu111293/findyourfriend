...
public class MapFragment extends BaseContainerFragment implements
	...
	private void afterViewCreate(View view) {
		...
		// load strangers
		loadStrangers(SettingManager.STRAGNER_DISTANCE);
	...
	private void loadStrangers(final double radius) {
		...
		getmMap().addCircle(
				(new CircleOptions())
						...
						.radius(radius).strokeWidth(2).strokeColor(0x55ffffff));
	...
	private void scrollForFriendList(final int toId) {
		avatarListView.postDelayed(new Runnable() {
			...
			public void run() {
				...
				//
				// Log.i(TAG, "disbefore " + disBefore + "#disaftert " +
				// disAfter);
			}
		}, 100);
	}
	...
	private void changeVisibilityView() {
		...
		// show Toast
		if (isToastPosition) {
			FrameLayout.LayoutParams layoutPrms = (FrameLayout.LayoutParams) textViewAddress
					.getLayoutParams();
	...
	private void showAlertRemoveHistory() {
		...
		Utility.showDialog(Utility.WARNING, dialog, "Xóa lịch sử",
				"Bạn có chắc muốn xóa tất cả lịch sử di chuyển?",
